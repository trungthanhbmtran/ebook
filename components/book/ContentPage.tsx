...
const ContentPage = ({ pageData, pageIndex, runningHeader, lang, onProjectClick }: ContentPageProps) => {
    ...
    const spineCurvature = isVi ? (
        <>
            <div className="absolute right-0 top-0 bottom-0 w-16 bg-gradient-to-l from-black/20 to-transparent pointer-events-none z-30"></div>
            <div className="absolute right-0 top-0 bottom-0 w-[2px] bg-black/10 pointer-events-none z-30"></div>
        </>
    ) : (
        <>
            <div className="absolute left-0 top-0 bottom-0 w-16 bg-gradient-to-r from-black/20 to-transparent pointer-events-none z-30"></div>
        ...
        <div className={`page-light bg-white w-full flex flex-col h-full relative ${wrapperShadow}`} style={{ backgroundColor: '#ffffff' }}>
            <div className={`flex-1 flex flex-col h-full relative z-20 ${wrapperPadding}`}>
                <div className="bg-[#5C3A21] w-full py-2 md:py-4 shrink-0 border-b-2 border-[#8A5A35] rounded-t-sm">
                    <h2 className="text-2xs md:text-sm font-bold text-white uppercase tracking-[0.05em] md:tracking-[0.1em] text-center px-2 md:px-4 leading-snug line-clamp-1">{runningHeader}</h2>
                </div>
                        ...
                        {pageData.map((item, itemIdx) => {
                            if (item.isMacroHeader) {
                                ...
                                return (
                                    <div key={`${lang}-macro-${itemIdx}`} className="col-span-2 relative overflow-hidden mb-1 md:mb-1.5 shrink-0">
                                        <div className="bg-gradient-to-r from-[#5C3A21] to-[#8A5A35] px-3 py-2 md:px-5 md:py-3 rounded-lg shadow-sm border-l-4 border-[#E5D3C1] flex justify-between items-center gap-2">
                                            <div className="flex-1">
                                                <div className="flex items-center gap-1 md:gap-2 mb-0.5 md:mb-1">
                                                    <span className="text-3xs md:text-2xs font-bold text-[#E5D3C1] uppercase tracking-[0.1em] md:tracking-[0.2em]">{labels.macroSpan}</span>
                                                </div>
                            ...
                            return (
                                <div key={`${lang}-proj-${item.id || itemIdx}`} className="relative bg-white rounded-xl border border-[#CBA365]/30 flex flex-col group h-[250px] md:h-[270px] shrink-0 overflow-hidden shadow-sm hover:shadow-[0_8px_25px_rgba(138,90,53,0.15)] hover:-translate-y-0.5 transition-all duration-300">
                                    {/* Accent top border */}
                                    <div className="h-1 w-full bg-gradient-to-r from-[#8A5A35] to-[#CBA365]"></div>
